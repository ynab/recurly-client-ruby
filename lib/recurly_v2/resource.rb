...
module RecurlyV2
  # The base class for all RecurlyV2 resources (e.g. {Account}, {Subscription},
  # {Transaction}).
  #
  ...
  # {RecurlyV2::Account} as an example.
  #
  # === Creating a Record
  #
  # You can instantiate a record before attempting to save it.
  #
  #   account = RecurlyV2::Account.new :first_name => 'Walter'
  #
  ...
  #
  #   RecurlyV2::Account.create(
  #     :first_name   => 'Jesse'
  #     :last_name    => 'Pinkman'
  #     :account_code => 'capn_cook'
  #   )
  #   # => #<RecurlyV2::Account account_code: "capn_cook" ...>
  #
  # You can use alternative "bang" methods for exception control. If the record
  # fails to save, a RecurlyV2::Resource::Invalid exception will be raised.
  #
  #   begin
  #     account = RecurlyV2::Account.new :first_name => 'Junior'
  #     account.save!
  #   rescue RecurlyV2::Resource::Invalid
  #     p account.errors
  #   end
  #
  ...
  #
  #   begin
  #     RecurlyV2::Account.create! :first_name => 'Skylar', :last_name => 'White'
  #   rescue RecurlyV2::Resource::Invalid => e
  #     p e.record.errors
  #   end
  #
  # === Fetching a Record
  ...
  #
  #   account = RecurlyV2::Account.find 'better_call_saul'
  #   # => #<RecurlyV2::Account account_code: "better_call_saul" ...>
  #
  # If the record doesn't exist, a RecurlyV2::Resource::NotFound exception will
  # be raised.
  #
  # === Updating a Record
  #
  ...
  #   # => true
  #
  # (A bang method, update_attributes!, will raise RecurlyV2::Resource::Invalid.)
  #
  ...
  class Resource
    require 'recurly_v2/resource/errors'
    require 'recurly_v2/resource/pager'
    require 'recurly_v2/resource/association'

    # Raised when a record cannot be found.
    #
    # @example
    #   begin
    #     RecurlyV2::Account.find 'tortuga'
    #   rescue RecurlyV2::Resource::NotFound => e
    ...
    # @example
    #   begin
    #     RecurlyV2::Account.create! :first_name => "Flynn"
    #   rescue RecurlyV2::Resource::Invalid => e
    ...
    class << self
      ...
      # @example
      #   RecurlyV2::Account.name # => "Account"
      def resource_name
      ...
      #   class.
      # @example
      #   RecurlyV2::Account.collection_name # => "accounts"
      def collection_name
      ...
      # @example
      #   RecurlyV2::Account.member_name # => "account"
      def member_name
      ...
      # @example
      #   RecurlyV2::Account.member_path "code" # => "accounts/code"
      #   RecurlyV2::Account.member_path nil    # => "accounts"
      ...
      #   {RecurlyV2::API::NotModified} if the pager's loaded page content has
      #   not changed
      # @example Fetch 50 records and iterate over them
      #   RecurlyV2::Account.paginate(:per_page => 50).each { |a| p a }
      # @example Fetch records before January 1, 2011
      #   RecurlyV2::Account.paginate(:cursor => Time.new(2011, 1, 1))
      ...
      def scopes
        @scopes ||= RecurlyV2::Helper.hash_with_indifferent_read_access
      end
      ...
      # @see Pager#paginate
      # @example
      #   RecurlyV2::Account.find_each { |a| p a }
      # @example With sorting and filter
      #   opts = {
      #     begin_time: DateTime.new(2016,1,1),
      #     sort: :updated_at
      #   }
      #   RecurlyV2::Account.find_each(opts) do |a|
      #     puts a.inspect
      #   end
      ...
      # @example
      #   RecurlyV2::Account.find "heisenberg"
      #   # => #<RecurlyV2::Account account_code: "heisenberg", ...>
      ...
      def from_response(response)
        ...
        when %r{application/xml}
          ...
          raise RecurlyV2::Error, "Content-Type \"#{content_type}\" is not accepted"
        end
      end
      ...
      def from_xml(xml)
        ...
        xml.each_element do |el|
          ...
          if association = find_association(el.name)
            ...
            resource_class = RecurlyV2.const_get(class_name)
      ...
      def has_one(member_name, options = {})
        ...
        associations_helper.module_eval do
          ...
          if options.key?(:readonly) && options[:readonly] == false
            associated = RecurlyV2.const_get Helper.classify(member_name), false
      ...
      def find_resource_class(name)
        ...
        if RecurlyV2.const_defined?(resource_name, false) && RecurlyV2.const_get(resource_name, false).instance_of?(Class)
          RecurlyV2.const_get(resource_name, false)
        end
      end
    end
    ...
    # Is the record new (i.e., not saved on RecurlyV2's servers)?
    #
    # @return [true, false]
    # @see #persisted?
    # @see #destroyed?
    ...
    # Has the record persisted (i.e., saved on RecurlyV2's servers)?
    #
    # @return [true, false]
    # @see #new_record?
    # @see #destroyed?
    ...
    def as_json(options = nil)
      attributes.reject { |k, v| v.is_a?(RecurlyV2::Resource::Pager) }
    end
    ...
    # @example
    #   account.read_link :billing_info # => <RecurlyV2::BillingInfo>
    ...
    # @example
    #   RecurlyV2::Account.new(:account_code => 'code').to_xml
    ...
    # @example
    #   account = RecurlyV2::Account.new
    #   account.save! # raises RecurlyV2::Resource::Invalid
    #   account.account_code = 'account_code'
    #   account.save! # => true
    # @see #save
    def save!
    ...
    #   (if persisted).
    # @example
    #   RecurlyV2::Account.new(:account_code => "account_code").uri # => nil
    #   RecurlyV2::Account.find("account_code").uri
    ...
    # (if the record does not persist on RecurlyV2).
    # @raise [NotFound] The record cannot be found.
    # @example
    #   account = RecurlyV2::Account.find account_code
    #   race_condition = RecurlyV2::Account.find account_code
    #   account.destroy        # => true
    #   account.destroy        # => false (already destroyed)
    #   race_condition.destroy # raises RecurlyV2::Resource::NotFound
    def destroy
    ...
    def fetch_associated(name, value, options = {})
      ...
      when Hash
        ...
        RecurlyV2.const_get(associated_class_name, false).send(:new, value)
